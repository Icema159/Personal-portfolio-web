...
const images = { "Developer Knowledge Hub": "devhub.png", "AriArt Website": "ariart.png" };
export default function EditorialProjects() {
 ...
 return <section id="projects" className="editorial-work"><div className="work-inner">
  ...
  {(all ? ordered : ordered.slice(0,4)).map((project,index) => <article className="work-item" key={project.title}>
   ...
   <div id={`project-detail-${index}`} className="work-detail" hidden={expanded!==index}>
    {images[project.title] && <img src={`${import.meta.env.BASE_URL}assets/${images[project.title]}`} alt={`${project.title} interface preview`} loading="lazy"/>}
 ...
 </div>{active && !reduce && <MotionDiv className="floating-preview" aria-hidden="true" style={{x:springX,y:springY}}>{images[active.title] ? <img src={`${import.meta.env.BASE_URL}assets/${images[active.title]}`} alt=""/> : <div className="preview-description"><span>{active.category}</span><strong>{active.title}</strong><p>{active.tags.slice(0,4).join(" / ")}</p></div>}<span className="preview-label">Explore project <ArrowUpRight size={14}/></span></MotionDiv>}</section>;
}
